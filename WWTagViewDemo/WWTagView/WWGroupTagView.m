...
#import "WWGroupTagView.h"

@interface WWTagViewCheckBoxButton :UIButton
...
@implementation WWTagViewCheckBoxButton
...
- (void)layoutTags {
    ...
    for (int i = 0; i < _tagButtons.count; i ++) {
        WWTagViewCheckBoxButton *tagButton = _tagButtons[i];
...
// 添加标签按钮
- (WWTagViewCheckBoxButton *)tagButtonWithTag:(NSString *)tag {
    
    WWTagViewCheckBoxButton *tagBtn = [[WWTagViewCheckBoxButton alloc] init];
 ...
 */
- (void)handlerTagButtonEvent:(WWTagViewCheckBoxButton*)sender {
...
- (void)addTagToLast:(NSString *)tag {
    ...
    if (result.count == 0) {
        ...
        WWTagViewCheckBoxButton* tagButton = [self tagButtonWithTag:tag];
...
- (void)setDeleteString:(NSString *)deleteString {
    ...
    for (int i = 0; i < self.tagStrings.count; i ++) {
        if ([deleteString isEqualToString:self.tagStrings[i]]) {
            WWTagViewCheckBoxButton *button = self.tagButtons[i];
            button.selected = NO;
            return;
        }
    }
