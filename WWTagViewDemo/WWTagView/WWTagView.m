...
@interface WWCheckBoxButton :UIButton
/**
 *  未被选中时标签的背景颜色
 */
...
@implementation WWCheckBoxButton
...
@interface WWTagView() <UITextFieldDelegate>
 ...
 *  被选中的标签
 */
@property (strong, nonatomic) WWCheckBoxButton *selectedBtn;
...
- (void)commonInit {
    ...
    {
        // 默认的标签
        UITextField* tf = [[WWTextField alloc] initWithFrame:CGRectMake(0, 0, _tagWidht, _tagHeight)];
...
- (NSMutableArray *)tagStrings {
      ...
      switch (_type) {
        case WWTagView_Type_Edit: {
            
            return _tagStrings;
        }
            break;

        case WWTagView_Type_Selected: {
            
            [_tagStringsSelected removeAllObjects];
            for (WWCheckBoxButton *button in _tagButtons) {
                if (button.selected) {
...
- (void)layoutTagviews {
    ...
    for (int i = 0; i < _tagButtons.count; i ++) {
        WWCheckBoxButton *tagButton = _tagButtons[i];
    ...
    _tfInput.hidden = (_type != WWTagView_Type_Edit);
    if (_type == WWTagView_Type_Edit) {
...
//        [_tfInput setValue:_inputPlaceHolderTextColor forKeyPath:@"_placeholderLabel.textColor"];
        _tfInput.attributedPlaceholder = [[NSAttributedString alloc] initWithString:_tfInput.placeholder attributes:@{NSForegroundColorAttributeName: _inputPlaceHolderTextColor}];
...
- (WWCheckBoxButton *)tagButtonWithTag:(NSString *)tag {
    
    WWCheckBoxButton *tagBtn = [[WWCheckBoxButton alloc] init];
...
#pragma mark - 按钮的点击事件
- (void)handlerTagButtonEvent:(WWCheckBoxButton *)sender {
...
- (void)addTagToLast:(NSString *)tag {
    ...
    if (result.count == 0) {
        ...
        WWCheckBoxButton *tagButton = [self tagButtonWithTag:tag];
...
#pragma mark - 删除标签
- (void)handlerButtonAction:(WWCheckBoxButton *)tagButton {
...
- (void)deleteItemClicked:(WWCheckBoxButton *)sender {
...
- (void)setType:(WWTagView_Type)type {
