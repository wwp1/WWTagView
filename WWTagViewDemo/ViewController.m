...
@interface ViewController ()<WWTagViewDelegate,WWGroupTagViewDelegate, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
...
- (void)addSubviews {
    ...
    inputTagView = [[WWTagView alloc] initWithFrame:CGRectMake(0, 0,self.view.width ,self.inputHeight)];
...
- (WWGroupTagView *)addHistoryViewTagsWithCGRect:(CGRect)rect andIndex:(NSIndexPath *)indexPath{
    
    
    WWGroupTagView *tagView = [[WWGroupTagView alloc] initWithFrame:rect];
...
- (void)deleteBtnClick:(NSString *)string {
    ...
    for (int j = 0; j < self.dataArr.count; j ++) {
        ...
        if (result.count != 0) {
            // 获取到对应的分组标签, 改变其标签的状态
            WWGroupTagView *tagView = [self.view viewWithTag:j + 1000];
